  ...
  useEffect(() => {
    const syncHeader = () => {
      ...
      if (window.scrollY > 12) {
        bar.classList.add("bg-white/90", "backdrop-blur", "shadow-soft");
      } else {
        bar.classList.remove("bg-white/90", "backdrop-blur", "shadow-soft");
      }
    };
    syncHeader();
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 antialiased">
      <header id="site-header" className="fixed inset-x-0 top-0 z-50 transition-all duration-300">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div
            ref={headerBarRef}
            className="mt-3 flex items-center justify-between rounded-2xl px-4 py-3 transition-all duration-300 md:px-6"
          >
            <a href="#" className="text-lg font-semibold tracking-tight">
              Dry<span className="text-brand-600">Zone</span>
            </a>
      ...
      <main className="flex-1">
        ...
        <section id="contact" className="mx-auto max-w-5xl px-4 pb-24 sm:px-6 lg:px-8">
          <div className="rounded-3xl bg-white p-8 shadow-softer sm:p-10">
            ...
            <form className="grid gap-4 md:grid-cols-4" noValidate onSubmit={onLeadSubmit}>
              ...
              <input
                ...
                className="rounded-xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-500"
              />
              ...
              <select
                id="lead-service"
                name="service"
                className="rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-brand-500"
              ...
              <button
                type="submit"
                disabled={submitting}
                className="rounded-xl bg-slate-900 px-5 py-3 font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-70"
              >
                Отправить
              </button>
            </form>
